...
public class KebabUtilities {
	...
	public static void updateKebab(int kebabToUpdateId, UpdateKebabRequest updatedKebab) {
		Kebab kebab = convertKebabReqToKebab(updatedKebab);
		kebabDao.updateKebab(kebabToUpdateId, kebab);
	}
	...
	public static Kebab convertKebabReqToKebab(AddKebabRequest addKebabRequest) {
		...
		try {
			...
			Base kebabBase = basesDao.getBaseByName(entityManager, addKebabRequest.getBase());
			Meat kebabMeat = meatsDao.getMeatByName(entityManager, addKebabRequest.getMeat());
			...
			saucesNames.forEach(sauceName -> {
				Sauce sauce = saucesDao.getSauceBySauceName(entityManager, sauceName);
