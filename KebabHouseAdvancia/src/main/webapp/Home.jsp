...
<%@ page
	import="com.advancia.daos.*, com.advancia.models.kebabComponents.*, com.advancia.models.*, java.util.*"%>
<!DOCTYPE html>
<html>
<head>
...
<path fill="#FFB300"
	...
	<section
		...
		<div class="flex justify-center items-center">
			<div class="customKebabBuildginForm">
				...
				<form class="flex h-full justify-center gap-4"
					action="home" method="POST">
					<input name="customKebabOperations" type="hidden" value="ADD"> <input
				...
				<table
					...
					<tbody>
						...
						<tr
							...
							<td scope="row"
								class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
								<input name="kebabId" type="hidden" value="<%=customKebab.getKebabId()%>">
								<input id="customKebabNameToChange" value="" class="hidden text-black border-black border-[2px]" required type="text"
							name="customKebabName">
					...
					<td id="editBaseTd" class=" hidden base -600 border-[2px]">
						...
						%>
							...
							<td id="editMeatTd" class=" hidden meat h-full -600 border-[2px]">
						<h3>Select Your Meat</h3>
						<%	
						if (meatsList != null) {
							for (Meat meat : meatsList) {
						%>
						<input type="radio" id="selectmeatselectbase<%=meat.getMeatId()%>td"
							name="meat" value="<%=meat.getMeatName()%>"> <label
						...
						%>
							...
							<td id="editIngredientsTd" class="hidden ingredients h-full -600 border-[2px]">
						<h3>Select Your Ingredients</h3>
						<%
						if (ingrList != null) {
							for (Ingredient ingredient : ingrList) {
						...
						<input type="checkbox"
							...
							name="ingredients[]" value="<%=ingredient.getIngredientName()%>">
						<label for="selectIngredient<%=ingredient.getIngredientId()%>td"><%=ingredient.getIngredientName()%></label>
						<br>
						<%
						}
						}
 ...
 %>
							...
							<td id="editSaucesTd" class="sauces hidden h-full -600 border-[2px]">
						<h3>Select Your Sauces</h3>
						<%
						if (saucesList != null) {
							for (Sauce sauce : saucesList) {
						%>
						<input type="checkbox" id="selectSauces<%=sauce.getSauceId()%>td"
							name="sauces[]" value="<%=sauce.getSauceName()%>"> <label
 ...
 %>
							...
							<td class="px-6 py-4">
								<%
								List<Allergen> customKebabAllergensList = customKebab.getAllergen();
 ...
 %>
							...
							<td class="flex items-center px-6 py-4">
									<input name="customKebabOperations" value="UPDATE" type="hidden">
									<button type="submit" id="saveButton"
										class=" hidden font-medium text-blue-600 dark:text-blue-500 hover:underline" onclick="toggleDivs()">Save</button>
								</form>
									<button type="submit" id="editButton"
										class=" font-medium text-blue-600 dark:text-blue-500 hover:underline" onclick="toggleDivs()">Edit</button>
								<form action="home" method="POST">
									<input type="hidden" name="kebabId" value="<%=customKebab.getKebabId()%>">
									<input name="customKebabOperations" value="DELETE" type="hidden">										
									<button 
  ...
  <div class="toast-content">
    ...
    <div class="message">
      ...
      <span class="text text-2">Kebab Name Exist Already</span>
    </div>
  </div>
  <div class="progress"></div>
</div>

<button id="alertBoxButton" onclick="loginFailed()">AlertBox</button>
